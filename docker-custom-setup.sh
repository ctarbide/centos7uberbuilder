...
exports=
for i in `git config -f "${docker_config}" --get-all docker.run.exports`; do
    perl -le'exit($ARGV[0] !~ m{^/})' -- "${i}" || die 1 "error: export ${i} must be an absolute path"
    i=`strip_r_slash "${i}"`
    exports="${exports} ${i#/}"
done
exports=${exports# }
...
echo "cleaning up the old environment at ${root_dir}"

rm -rf "${root_dir}"

mkdir -p "${root_dir}"

echo "setting up a customized environment for ${imagename}"
...
tar -C / -cf - ${EXPORTS}' | tar -C "${root_dir}" -xf -
